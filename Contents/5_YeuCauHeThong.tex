...
\paragraph{Thu thập dữ liệu giao thông}
\begin{itemize}
    \item \textbf{FR1.1}: Hệ thống cho phép tải lên dữ liệu lịch sử giao thông.
    \item \textbf{FR1.2}: Hỗ trợ nhiều định dạng dữ liệu đầu vào: JSON, GeoJSON, CSV.
    \item \textbf{FR1.3}: Kiểm tra cấu trúc và tính hợp lệ của dữ liệu (schema validation).
\end{itemize}

\paragraph{Tiền xử lý dữ liệu}
\begin{itemize}
    \item \textbf{FR2.1}: Loại bỏ dữ liệu thiếu, sai định dạng hoặc bất thường.
    \item \textbf{FR2.2}: Chuẩn hoá các thuộc tính như tốc độ, thời gian di chuyển, timestamp.
    \item \textbf{FR2.3}: Ghép nối dữ liệu với mạng lưới đường (map-matching) khi cần.
\end{itemize}

\paragraph{Phân tích mạng lưới giao thông}
\begin{itemize}
    \item \textbf{FR3.1}: Xây dựng mô hình đồ thị gồm các nút (node) và cạnh (segment).
    \item \textbf{FR3.2}: Tính toán các thông số của mạng đường: độ dài, phân cấp đường (FRC), các chỉ số trung tâm.
    \item \textbf{FR3.3}: Xác định “điểm nóng” (hotspot) tắc nghẽn theo thời gian.
\end{itemize}

\paragraph{Phân tích mối liên hệ giữa các đoạn đường}
\begin{itemize}
    \item \textbf{FR4.1}: Tính toán mức độ tương quan giữa các đoạn đường theo thời gian.
    \item \textbf{FR4.2}: Đo độ tương đồng dạng thời gian bằng kỹ thuật Dynamic Time Warping (DTW).
    \item \textbf{FR4.3}: Phát hiện hiện tượng lan truyền tắc nghẽn trong mạng lưới.
\end{itemize}

\paragraph{Dự báo tình trạng giao thông}
\begin{itemize}
    \item \textbf{FR5.1}: Xây dựng tập dữ liệu theo chuỗi thời gian (time-series).
    \item \textbf{FR5.2}: Huấn luyện mô hình dự báo như LSTM, GCN, T-GCN,...
    \item \textbf{FR5.3}: Đánh giá độ chính xác mô hình bằng các thước đo MAE, RMSE.
\end{itemize}

\paragraph{Trực quan hoá dữ liệu}
\begin{itemize}
    \item \textbf{FR6.1}: Hiển thị bản đồ mạng giao thông bằng dữ liệu OSM hoặc TomTom.
    \item \textbf{FR6.2}: Trực quan tốc độ trung bình theo thời gian của từng segment.
    \item \textbf{FR6.3}: Hiển thị biểu đồ chuỗi thời gian.
    \item \textbf{FR6.4}: Hiển thị ma trận tương quan giữa các đoạn đường.
    \item \textbf{FR6.5}: Cho phép thao tác xem dữ liệu trên dashboard (ví dụ: Streamlit).
\end{itemize}

\paragraph{Xuất báo cáo}
\begin{itemize}
    \item \textbf{FR7.1}: Xuất kết quả phân tích ra các định dạng CSV, JSON hoặc PDF.
    \item \textbf{FR7.2}: Cho phép tải xuống kết quả.
\end{itemize}
...
\paragraph{Hiệu năng}
\begin{itemize}
    \item Thời gian xử lý dữ liệu phải đảm bảo hiệu quả (ví dụ: xử lý 1 MB dữ liệu trong vòng dưới 5 giây).
...
\begin{itemize}
    \item Mã nguồn phải được tổ chức thành các module độc lập gồm: thu thập dữ liệu, tiền xử lý, phân tích, mô hình hoá và trực quan hoá.
\end{itemize}
...
Hệ thống cần thu thập dữ liệu mô tả tình trạng giao thông đô thị từ một hoặc nhiều nguồn khác nhau (API giao thông, cảm biến, dữ liệu mở, dữ liệu bản đồ số, v.v.). Các nhóm dữ liệu chính bao gồm:
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Khoảng ngày quan sát (\textit{date ranges}).
        \item Khoảng thời gian trong ngày (\textit{time sets}).
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Giảm thiếu dữ liệu lỗi hoặc thiếu:} hạn chế phần tử bị trống hoặc sai lệch.
    \item \textbf{Có thể truy vết:} biết rõ nguồn và thời điểm thu thập.
\end{itemize}

\subsubsubsection{Yêu cầu về cập nhật dữ liệu}
\begin{itemize}
    \item Hệ thống cần hỗ trợ nhập và cập nhật dữ liệu từ nhiều nguồn khác nhau trong tương lai.
    \item Cho phép cập nhật định kỳ khi có dữ liệu mới.
\end{itemize}